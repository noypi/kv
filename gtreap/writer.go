...
func (w *Writer) NewBatch() kv.KVBatch {
	return kv.NewEmulatedBatch(w.s.mo)
}

func (w *Writer) NewBatchEx(options kv.KVBatchOptions) ([]byte, kv.KVBatch, error) {
	return make([]byte, options.TotalBytes), w.NewBatch(), nil
}

func (w *Writer) ExecuteBatch(batch kv.KVBatch) error {

	emulatedBatch, ok := batch.(*kv.EmulatedBatch)
	if !ok {
